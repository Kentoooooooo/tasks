	...
	for(s = amp_min_int; s <= amp_max_int; s++) {
		...
		if(out == 1) {
			if(para_flag == 2) {
				...
				fprintf(outputfp[0], "Index\tInput Santa Fe\tTarget\tTraining result\tNode vc difference\n");
				for(i = 0; i < rc.trainingNum; i++) {
					...
					fprintf(outputfp[0], "%d\t%e\t%e\t%e\t%e\n", i, rc.input_train[i], rc.target_train[i], rc.output_train[i], sqrt(node_vc_difference));
				}
				fclose(outputfp[0]);
				
				EvaluateReservoirTest(&rc);
				outputfp[0] = fopen("rc_test_result.txt", "w");
				fprintf(outputfp[0], "Index\tInput Santa Fe\tTarget\tTest result\tNode vc difference\tVar of nodes\n");
				for(i = 0; i < rc.testNum; i++) {
					...
					fprintf(outputfp[0], "%d\t%e\t%e\t%e\t%e\t%e\n", i, rc.input_test[i], rc.target_test[i], rc.output_test[i], sqrt(node_vc_difference), hist[0].variance);
				}
				fclose(outputfp[0]);
			}
			break;
