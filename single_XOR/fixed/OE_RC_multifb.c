...
int main(int argc, char *argv[]) {
	...
	double param, param_min, param_max, param_interval, max_delay, t_transient, t_plot, tmp;
	...
	for(s = amp_min_int; s <= amp_max_int; s++) {
		...
		laser.x = 0.001;
		laser.y = 0.001;
		for(i = 0; i < laser.descretized_num[laser.long_fb]; i++) laser.xdelay[i] = 0.001;
		for(i = 0; i < laser.fbNum; i++) laser.fbIndex[i] = laser.descretized_num[laser.long_fb] - laser.descretized_num[i];
		init_gen_rand(222);
		...
		for(i = 1; i <= calc_num; i++) {
			...
			// if(rc.maskIndex % 2 == 0) tmp = rc.inputSantaFe[rc.inputIndex];
			// else tmp = (255 - rc.inputSantaFe[rc.inputIndex]);
			
			rc.input_signal = rc.gamma * rc.mask[rc.maskIndex] * rc.inputSantaFe[rc.inputIndex];
			// rc.input_signal = rc.gamma * rc.mask[rc.maskIndex] * tmp;
...
double *MakeMask(int mask_num) {
	...
	for(i = 0; i < mask_num; i++) {
		...
		// if(rand_num == 0) mask[i] = 1.00;
		// if(rand_num == 1) mask[i] = 0.75;
		// if(rand_num == 2) mask[i] = 0.50;
		// if(rand_num == 3) mask[i] = 0.25;
	}
