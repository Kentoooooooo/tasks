...
struct parameter {
	double tau_L, tau_H, delay[2], beta, kappa, sigma, bias_pt, phi, epsilon;
};
...
struct Reservoir {
	int trainingNum, testNum, mask_num, node_num, allNum, mask_interval, node_interval, input_interval, mask_idx, node_idx, input_idx, delay_step;
	double gamma, *mask, **train_node, **test_node, *target_train, *target_test, *weight, *input_train, *input_test, *output_train, *output_test, input_signal, input_MZM, input_MZM_phi, nyu;
};
...
void MakeMask(struct Reservoir *rc, int seed);
...
int main(int argc, char *argv[]) {
	...
	struct cc_data cc[2];
	double param, param_min, param_max, param_interval, t_transient, t_start_plot, t_plot;
	double buff[8], *wfmData[8], node_vc_difference;
	int i, j, k, n, p, s, t, transient, calc_num, divi, out, para_flag, iteration, start_plot, plotnum;
	...
	if (argc == 2) {
		...
		fscanf(inputfp, "%lf %lf", &(laser[0].p.bias_pt), &(laser[1].p.bias_pt));
		fscanf(inputfp, "%lf %lf %lf", &param_min, &param_max, &param_interval);
		fscanf(inputfp, "%lf %lf %lf", &t_transient, &t_start_plot, &t_plot);
	...
	if(out == 1) {
		...
		printf("\t 13 --> Nonlinearity Nu\n");
		printf("\t 14 --> Memory Tau_d\n");
		printf("inputfp --> ");
		scanf("%d", &para_flag);
		...
		if(para_flag == 13) sprintf(buffer_ch, "Nonlinearity \\fn\\n");
		if(para_flag == 14) sprintf(buffer_ch, "Delay step \\ft\\n\\dD\\n");
	...
	laser[0].p.phi = laser[0].p.bias_pt * M_PI;
	laser[1].p.phi = laser[1].p.bias_pt * M_PI;
	...
	SetRC_Param(&rc, 0.2e-6, 50, 1);//mask interval, number of mask, ratio of node to mask
	
	transient = (int)(t_transient / dh + EPS);
	start_plot = (int)(t_start_plot / dh + EPS);
	plotnum = (int)(t_plot / dh + EPS);
	printf("\n-------------- parameters --------------\n");
	printf("Number of node: %d\n", rc.node_num);
	printf("Number of one mask point: %d\n", rc.node_interval);
	printf("Delay time 1: %e, Delay time 2: %e\n", laser[0].p.delay[0], laser[1].p.delay[0]);
	printf("beta : %e\n", laser[0].p.beta);
	printf("kappa 1 : %e, kappa 2 : %e\n", laser[0].p.kappa, laser[1].p.kappa);
	printf("sigma 1 : %e, sigma 2 : %e\n", laser[0].p.sigma, laser[1].p.sigma);
	printf("Bias point : %e\n", laser[0].p.bias_pt);
	printf("param_min : %e, param_max : %e, param_interval : %e\n", param_min, param_max, param_interval);
	printf("divi: %d\n", divi);
	printf("nu: %e\n", rc.nyu);
	printf("delay_step: %d\n", rc.delay_step);
	printf("gamma: %e\n", rc.gamma);
	...
	for(s = amp_min_int; s <= amp_max_int; s++) {
		...
		if(out == 2) {
			...
			if(para_flag == 13) {  //chg nyu
				rc.nyu = param;
				SetRC_Param(&rc, 0.2e-6, 50, 1);
				printf("\nNL strength... nu = %e\n", rc.nyu);
			}
			if(para_flag == 14) {  //chg delay_step
				rc.delay_step = param;
		...
		for(i = 1; i <= calc_num; i++) {
			...
			/***** amplitude modulation *****/
			/***** input bias modulation *****/
			// rc.input_signal = rc.mask[rc.mask_idx] + rc.gamma * rc.input_train[rc.input_idx];
			/***** input bias modulation *****/
			/***** feedback bias modulation *****/
			// rc.input_signal = rc.mask[rc.mask_idx];
			// laser[0].p.phi = M_PI * laser[0].p.bias_pt + M_PI / 2.0 * rc.gamma * rc.input_train[rc.input_idx];
			// laser[1].p.phi = M_PI * laser[1].p.bias_pt + M_PI / 2.0 * rc.gamma * rc.input_train[rc.input_idx];
			/***** feedback bias modulation *****/
		...
		for(i = 1; i <= calc_num; i++) {
			...
			/***** amplitude modulation *****/
			/***** input bias modulation *****/
			// rc.input_signal = rc.mask[rc.mask_idx] + rc.gamma * rc.input_test[rc.input_idx];
			/***** input bias modulation *****/
			/***** feedback bias modulation *****/
			// rc.input_signal = rc.mask[rc.mask_idx];
			// laser[0].p.phi = M_PI * laser[0].p.bias_pt + M_PI / 2.0 * rc.gamma * rc.input_test[rc.input_idx];
			// laser[1].p.phi = M_PI * laser[1].p.bias_pt + M_PI / 2.0 * rc.gamma * rc.input_test[rc.input_idx];
			/***** feedback bias modulation *****/
			rc.input_MZM = cos(rc.input_signal * M_PI / 4.0 + rc.input_MZM_phi);
			rc.input_MZM *= rc.input_MZM;
		...
		if(out == 1) {
			if(para_flag == 2) {
				...
				fprintf(outputfp[0], "Index\tInput data\tTarget\tTraining result\tNode vc difference\n");
				for(i = 0; i < rc.trainingNum; i++) {
					for(j = 0, node_vc_difference = 0.0; j < rc.node_num; j++) {
						node_vc_difference +=  (rc.train_node[184][j] - rc.train_node[i][j]) * (rc.train_node[184][j] - rc.train_node[i][j]);
						node_vc_difference +=  (rc.train_node[184][rc.node_num + j] - rc.train_node[i][rc.node_num + j]) * (rc.train_node[184][rc.node_num + j] - rc.train_node[i][rc.node_num + j]);
					}
					fprintf(outputfp[0], "%d\t%e\t%e\t%e\t%e\n", i, rc.input_train[i], rc.target_train[i + rc.delay_step], rc.output_train[i + rc.delay_step], sqrt(node_vc_difference));
				}
				fclose(outputfp[0]);
				
				EvaluateReservoirTest(&rc);
				outputfp[0] = fopen("rc_test_result.txt", "w");
				fprintf(outputfp[0], "Index\tInput data\tTarget\tTest result\tNode vc difference\tVar of nodes\n");
				for(i = 0; i < rc.testNum; i++) {
					...
					for(j = 0, node_vc_difference = 0.0; j < rc.node_num; j++) {
						...
						InputHistgram(&hist[0], rc.test_node[i][j]);
						InputHistgram(&hist[0], rc.test_node[i][rc.node_num + j]);
					}
					OutputHistgram(&hist[0], "0");
					fprintf(outputfp[0], "%d\t%e\t%e\t%e\t%e\t%e\n", i, rc.input_test[i], rc.target_test[i + rc.delay_step], rc.output_test[i + rc.delay_step], sqrt(node_vc_difference), hist[0].variance);
				}
				fclose(outputfp[0]);
			}
			break;
...
void SetRC_Param(struct Reservoir *rc, double mask_interval, int mask_num, int mask_node_ratio) {
	...
	init_gen_rand(2121);
	double *rand_temp;
	rand_temp = (double*)malloc(sizeof(double) * (rc->allNum + rc->delay_step));
	for(i = 0; i < rc->allNum + rc->delay_step; i++) rand_temp[i] = 2 * genrand_open_open() - 1;
	for(i = 0; i < rc->trainingNum; i++) {
		rc->input_train[i] = rand_temp[i + rc->delay_step];
		rc->target_train[i] = sin(rc->nyu * rand_temp[i]);
	}
	for(i = 0; i < rc->testNum; i++) {
		rc->input_test[i] = rand_temp[i + rc->trainingNum + rc->delay_step];
		rc->target_test[i] = sin(rc->nyu * rand_temp[i + rc->trainingNum]);
	}
...
void CalcLaserEOfeedback(struct LaserEOfeedback *laser, double input_MZM) {
	...
	for(i = 0; i < 2; i++) {
		...
		feedback[i] = laser[i].p.beta * 2.0 * input_MZM * cosTheta * cosTheta;// �ǉ���
	}
